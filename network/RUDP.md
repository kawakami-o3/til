...
RUDP はクライアントとサーバーの操作双方をサポートしなければならない。
これにより両方が接続を活発に開始できる。

RUDP は 1 オクテット長のデータから 2 オクテットに展開し、sequence と acknowledge フィールドを
抽出できなければならない。これにより、255バッファーよりも大きな転送ウィンドウを確保できる。

ネットワーク効率の向上を目指して、Nagle アルゴリズムの利用を試みるとよいかもしれない。
